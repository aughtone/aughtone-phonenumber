...
Hardens `parse` against alphabetic and marker input, aligns the extension model with RFC 3966, and trims the target set. In the default mode a vanity number is refused rather than keypad-folded, a stray word or unknown extension marker is never folded into the number, and an extension is separated from a post-dial string. With `libphonenumberCompat = true` the covered surface stays byte-identical to libphonenumber 9.0.39, and `formatToE164()` is unchanged for every number that already parsed — so existing E.164 tokens are stable. Review **Removed** (dropped native targets) and **Upgrading from 0.0.3** before adopting.
...
- **`ErrorType` gained `ALPHA_NUMBER_DISALLOWED`.** A Kotlin `when` over `ErrorType` that was exhaustive will stop compiling until the new branch (or an `else`) is added — intentional, so a vanity refusal is handled deliberately.
- **`PhoneNumber` gained `droppedText` and `postDialString`** (both read-only, never in `formatToE164()`). Code that copies or exhaustively destructures `PhoneNumber` may need updating.
- **Default-mode `parse` changed for alphabetic and marker input** (all reversible with `libphonenumberCompat = true`, which reproduces 0.0.3/upstream): a vanity number is now refused unless `allowVanityNumbers = true`; an unrecognised alphabetic marker after a valid number is dropped (see `droppedText`) rather than folded, and `allowUnknownExtensions = true` extracts its digits as the extension; a `,`/`;`/`#`/`~` sequence is now `postDialString`, not `extension`. None of these change `formatToE164()` for a number that already parsed — they change refusals and which field a sub-address lands in.
- **Dropped native targets** `iosX64`, `tvosArm64`, `tvosSimulatorArm64`, `watchosArm32` (see **Removed**). A build pinning one of these must move to a supported target.
