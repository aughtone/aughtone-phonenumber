...
> **Caveat — vanity conversion folds letters into digits.** Matching upstream, a run of three or more letters is keypad-converted (`1-800-FLOWERS` → `1-800-3569377`). Called directly on messy input, this means incidental words become part of the number rather than being rejected, so a stray word can produce a valid-looking wrong number. Strip or reject alphabetic input before `parse` if that is a risk for you.
